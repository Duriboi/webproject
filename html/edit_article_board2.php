...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $sql_update = "UPDATE board2_posts SET title = ?, content = ? WHERE id = ? AND user_id = ?";
    $stmt_update = $conn->prepare($sql_update);
    $stmt_update->bind_param("ssii", $title, $content, $article_id, $_SESSION['user_id']);
    $stmt_update->execute();

    header("Location: view_article_board2.php?id=$article_id");
    exit();
}
...
<form action="edit_article_board2.php?id=<?php echo $article_id; ?>" method="post">
